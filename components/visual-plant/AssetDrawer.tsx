      ...
      >
        ...
        <div className="p-6 border-b border-industrial-700 flex justify-between items-start bg-industrial-900/50">
          <div>
             ...
             <p className="text-sm text-industrial-500">{machine.type} High-Precision Unit</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div className="bg-industrial-800/80 rounded-xl p-4 h-full">
               <div className="flex justify-between items-center mb-3">
                 <h3 className="font-bold text-white flex items-center gap-2">
                   <span className="text-indigo-400">✦</span> Gemini Diagnostics
                 </h3>
                 {!analysis && (
                   <button 
               ...
               {analysis && (
                 <div className="space-y-3 animate-fadeIn">
                    <div className="flex justify-between items-end border-b border-white/10 pb-2">
                      <span className="text-xs text-industrial-400">Health Score</span>
                    ...
                    <div>
                      <span className="text-[10px] uppercase tracking-wider text-industrial-500 font-bold">Predicted Failure</span>
                      <p className="text-white text-sm mt-0.5">{analysis.predictedFailure}</p>
                    </div>
                    <div>
                      <span className="text-[10px] uppercase tracking-wider text-industrial-500 font-bold">Recommendation</span>
          ...
          <div>
            <h3 className="text-sm font-bold text-industrial-400 uppercase mb-3 flex items-center gap-2">
              <Box size={14} /> Critical Spares (Kardex)
            </h3>
              ...
              {criticalSpares.map((part, idx) => (
                <div key={idx} className="flex justify-between items-center p-3 bg-industrial-900 rounded border border-industrial-700">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-mono text-industrial-500">Qty: {part.qty}</span>
                    <span className={`w-2 h-2 rounded-full ${
                      part.status === 'IN_STOCK' ? 'bg-emerald-500' : 
          ...
          <div>
             <h3 className="text-sm font-bold text-industrial-400 uppercase mb-3 flex items-center gap-2">
               <Calendar size={14} /> Next Service
             </h3>
             <div className="bg-industrial-900 rounded-lg p-4 border border-industrial-700 flex items-center justify-between">
                <div>
                   <p className="text-white text-sm font-medium">Preventive R-MANT-02</p>
                   <p className="text-xs text-industrial-500">Scheduled: {new Date(machine.nextMaintenance).toLocaleDateString()}</p>
                </div>
                <div className="text-right">
                   ...
                   <span className="text-[10px] text-industrial-500 uppercase">Days Left</span>
                </div>
             </div>
          </div>
