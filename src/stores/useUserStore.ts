...
interface UserState {
  ...
  addUser: (email: string, fullName: string, role: UserRole, jobTitle: string) => Promise<void>;
...
export const useUserStore = create<UserState>((set, get) => ({
  ...
  addUser: async (email, fullName, role, jobTitle) => {
    set({ isLoading: true, error: null });
    try {
      const newUser = await UserSupabaseService.inviteUser(email, fullName, role, jobTitle);
      set((state) => ({ 
