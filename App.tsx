...
const ConfigurationPage = () => {
  const { machines, technicians, plantSettings, addMachine, updateMachine, addTechnician, updateSettings } = useMasterStore();
...
const MaintenanceFormPage = () => {
  ...
  // For edit, we might use URL params via useParams(), sticking to simple state for now or handling URL params if path is /orders/:id
  // But since MaintenanceForm handles 'initialData', let's assume creation mainly or edit passed via router state.
  // Ideally, we check URL param. BUT refactoring MaintenanceForm to load data is out of scope.
  // I will assume state passing for now or query params.

  // Simplification: We render MaintenanceForm.
  // It expects `initialMachineId`, `initialData`.
  ...
  return (
    <MaintenanceForm
      ...
      initialData={undefined} // Pass undefined or logic to load existing Order
    />
  );
};
